...
const scoreDOM = document.getElementById('hud-score');
const bestDOM = document.getElementById('hud-best');
const endScoreDOM = document.getElementById('endScore');
const endBestDOM = document.getElementById('endBest');

// Highscore — shares the arcade's localStorage convention (arcade-highscore-<gameId>)
// so this game's best score shows up consistently with every other game in the arcade.
const HIGHSCORE_KEY = 'arcade-highscore-crossy';
let highScore = Number(localStorage.getItem(HIGHSCORE_KEY)) || 0;
if (bestDOM) bestDOM.textContent = `BEST ${highScore}`;
...
function triggerGameOver() {
  if (isGameOver) return;
  isGameOver = true;
  if (endScoreDOM) endScoreDOM.textContent = `Score: ${currentLane}`;
  if (endBestDOM) endBestDOM.textContent = `Best: ${highScore}`;
...
function retry() {
  ...
  initaliseValues();
  if (scoreDOM) scoreDOM.textContent = 'SCORE 0';
...
window.addEventListener("keydown", event => {
  if (isGameOver) {
    // Matches the arcade controller's action buttons: A ('z') retries, B ('x') leaves.
    if (event.key === 'z' || event.key === 'Z') retry();
    else if (event.key === 'x' || event.key === 'X') window.location.href = '../index.html';
    return;
  }
  if (event.keyCode == '38') {
    move('forward');
...
// Resize to the framed container (not the full window) so the game stays inside
// the arcade's bordered canvas area instead of going fullscreen.
...
new ResizeObserver(resizeToContainer).observe(gameContainer);
  ...
  if(stepStartTimestamp) {
    ...
    if(moveDeltaTime > CONFIG.stepTime) {
      switch(moves[0]) {
        case 'forward': {
          currentLane++;
          reportScore(currentLane);
          break;
        }
        case 'backward': {
          currentLane--;
          if (scoreDOM) scoreDOM.textContent = `SCORE ${currentLane}`;
          break;
        }
        case 'left': {
          currentColumn--;
